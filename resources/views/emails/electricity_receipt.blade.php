...
<head>
    ...
    <title>Electricity Payment — A-Pay</title>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        /* Token highlight box */
        .token-box { background: linear-gradient(135deg, #00251a, #003d2a); border-radius: 16px; padding: 24px; margin-bottom: 28px; text-align: center; }
        .token-label { font-size: 11px; font-weight: 700; letter-spacing: 1.5px; text-transform: uppercase; color: rgba(255,255,255,0.4); margin-bottom: 12px; }
        .token-value { font-family: 'JetBrains Mono', monospace; font-size: 28px; font-weight: 600; color: #00e676; letter-spacing: 4px; margin-bottom: 8px; word-break: break-all; }
        .token-units { font-size: 13px; color: rgba(255,255,255,0.5); }
        .token-units strong { color: #fff; }
        ...
        .detail-value.status-pill { font-family: 'Sora', sans-serif; padding: 4px 12px; border-radius: 100px; font-size: 11px; font-weight: 700; }
...
<div class="wrapper">
    ...
    <div class="card">
        <div class="hero {{ $status == 'SUCCESS' ? 'success' : 'failed' }}">
            <div class="hero-label {{ $status == 'SUCCESS' ? 'success' : 'failed' }}">Electricity Payment</div>
            <div class="hero-amount"><span>₦</span>{{ number_format($amount, 2) }}</div>
            <div class="hero-subtitle">{{ $status == 'SUCCESS' ? 'Payment processed successfully' : 'Payment could not be completed' }}</div>
        ...
        <div class="body">
            ...
            <p class="greeting-sub">{{ $status == 'SUCCESS' ? 'Your electricity bill has been paid. Save your token below to power your meter.' : 'We were unable to process your payment. Your wallet has been refunded.' }}</p>

            @if($status == 'SUCCESS')
            <div class="token-box">
                <div class="token-label">⚡ Meter Token</div>
                <div class="token-value">{{ strtoupper($token) }}</div>
                <div class="token-units">Units: <strong>{{ strtoupper($units) }} kWh</strong></div>
            </div>
            @endif

            <div class="details-card {{ $status != 'SUCCESS' ? 'failed-card' : '' }}">
                <div class="details-header {{ $status == 'SUCCESS' ? 'success' : 'failed' }}">Payment Details</div>
                <div class="detail-row">
                    <span class="detail-label">Amount Paid</span>
                    <span class="detail-value {{ $status == 'SUCCESS' ? 'green' : 'red' }}">₦{{ number_format($amount, 2) }}</span>
                </div>
                <div class="detail-row">
                    <span class="detail-label">Meter Number</span>
                    <span class="detail-value">{{ $meterNumber }}</span>
                </div>
                <div class="detail-row">
                    <span class="detail-label">Provider</span>
                    <span class="detail-value">{{ strtoupper($provider) }}</span>
                </div>
                @if($status == 'SUCCESS')
                <div class="detail-row">
                    <span class="detail-label">Account Name</span>
                    <span class="detail-value">{{ $customer_name_m }}</span>
                </div>
                <div class="detail-row">
                ...
                <div class="detail-row">
                    ...
                    <span class="detail-value status-pill {{ $status == 'SUCCESS' ? 'success' : 'failed' }}">{{ $status == 'SUCCESS' ? '✓ Successful' : '✗ Failed' }}</span>
                </div>
            </div>
            ...
            <div class="refund-notice">
                ...
                <div class="refund-text"><strong>Wallet Refunded:</strong> ₦{{ number_format($amount, 2) }} has been restored to your A-Pay wallet. You can retry anytime.</div>
            </div>
            @endif
