...
<head>
    ...
    <title>Betting Top-up — A-Pay</title>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .brand-bar .logo { display: inline-block; background: linear-gradient(135deg, #00e676, #00bfa5); color: #000; font-family: 'Sora', sans-serif; font-weight: 700; font-size: 22px; letter-spacing: -0.5px; padding: 8px 24px; border-radius: 100px; }
...
<div class="wrapper">
    ...
    <div class="card">
        <div class="hero {{ $status == 'SUCCESS' ? 'success' : 'failed' }}">
            <div class="hero-label {{ $status == 'SUCCESS' ? 'success' : 'failed' }}">Betting Top-up</div>
            <div class="hero-amount"><span>₦</span>{{ number_format($amountPaid, 2) }}</div>
            <div class="hero-subtitle">{{ $status == 'SUCCESS' ? 'Top-up processed successfully' : 'Transaction could not be completed' }}</div>
        ...
        <div class="body">
            ...
            <p class="greeting-sub">{{ $status == 'SUCCESS' ? 'Your betting account has been topped up. Here\'s the transaction summary.' : 'We were unable to process your top-up. Your wallet has been refunded.' }}</p>
            <div class="details-card {{ $status != 'SUCCESS' ? 'failed-card' : '' }}">
                ...
                <div class="detail-row">
                    ...
                    <span class="detail-value {{ $status == 'SUCCESS' ? 'green' : 'red' }}">₦{{ number_format($amountPaid, 2) }}</span>
                </div>
                <div class="detail-row">
            ...
            <div class="refund-notice">
                ...
                <div class="refund-text"><strong>Wallet Refunded:</strong> ₦{{ number_format($amountPaid, 2) }} has been restored to your A-Pay wallet. You can retry anytime.</div>
            </div>
            @endif
            <div class="security-note">
                <span class="security-icon">🔐</span>
                <div class="security-text"><strong>Security Notice:</strong> A-Pay will never ask for your PIN, password, or OTP via email or WhatsApp. If you did not initiate this transaction, contact support immediately.</div>
            </div>
        </div>
        <div class="footer">
