...
<head>
    ...
    <title>Data Purchase — A-Pay</title>
...
<div class="wrapper">
    ...
    <div class="card">
        <div class="hero {{ $status == 'SUCCESS' ? 'success' : 'failed' }}">
            <div class="hero-label {{ $status == 'SUCCESS' ? 'success' : 'failed' }}">Data Purchase</div>
            <div class="hero-amount"><span>₦</span>{{ number_format($amount, 2) }}</div>
            <div class="hero-subtitle">{{ $status == 'SUCCESS' ? 'Data activated successfully' : 'Transaction could not be completed' }}</div>
        ...
        <div class="body">
            ...
            <p class="greeting-sub">{{ $status == 'SUCCESS' ? 'Your data purchase was successful. Here\'s the transaction summary.' : 'We were unable to process your data purchase. Your wallet has been refunded.' }}</p>
